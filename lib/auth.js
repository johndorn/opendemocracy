...
module.exports.verifyCertificate = function(req, res, next) {
	...
	if(certificateIsAuthorized(req.user, req.connection.getPeerCertificate())) { // key only signed if belonging to a user
		next();
	}
...
/**
	lib/auth.generateCertVerifyUserHash(password, cert, user)
 ...
 */
module.exports.generateCertVerifyUserHash = function(password, cert, user) {
	var sha = crypto.createHash(hashing_algo);
	sha.update(password);
